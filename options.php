...
?>
  <div class="wrap">
    ...
    <p><h2>2. Вставьте в текстовое поле данные с показаниями в следующем порядке (каждый Лицевой счет с данными с новой строки):<br>
...
<p>
<h2>Скачать файлы с переданными показаниями (manual - показания, заполненные вручную):</h2>
</p>  
<div class="select">
 <select id="select_pokaz" style="width: 100%;">
<?php
$files9 = glob( __DIR__ . '/pok/*.txt');
usort($files9, create_function('$a,$b', 'return filemtime($b) - filemtime($a);'));
foreach($files9 as $file9) {	
$file99 = basename($file9);
	echo '<option value="' . plugins_url( 'pok/' . $file99, __FILE__ ) . '">' . $file99 . '</option>';
    }	
?>		
</select>
   <button class="button" onclick="foo()">Скачать показания</button>
 </div> 
<script>
function foo()
{
 if( select_pokaz .value)
 {
 get_file_url(select_pokaz .value);
 }
}
...
<hr>
<p> 
<div class="active second plugin-version-author-uri">Pokazania IPU | Версия 1.0.0 | Автор: <a href="https://kodabra.unchikov.ru/">Elena Unchikova</a> | <a href="https://kodabra.unchikov.ru/pokazania-ipu/" aria-label="Посетите сайт плагина Pokazania IPU">Перейти на страницу плагина</a>
</div>
</p> 
<hr>
  </div>
